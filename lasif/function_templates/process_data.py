...
def processing_function(st, inv, processing_params, event):  # NOQA
    ...
    endtime = starttime + processing_params["dt"] * (processing_params["npts"] - 1)
    ...
    for tr in st:
        ...
        try:
            ...
            tr.remove_response(output="DISP", pre_filt=pre_filt, zero_mean=False, taper=False)
        except Exception as e:
            station = tr.stats.network + "." + tr.stats.station + ".." + tr.stats.channel
            msg = ("File  could not be corrected with the help of the "
                   "StationXML file '%s'. Due to: '%s'  Will be skipped.") % (station, e.__repr__()),
            raise LASIFError(msg)
        ...
        tr.filter("bandpass", freqmin=1.0/max_period, freqmax=1.0/min_period, corners=3,
                  zerophase=False)
        tr.detrend("linear")
        tr.detrend("demean")
        tr.taper(0.05, type="cosine")
        ...
        tr.interpolate(sampling_rate=1.0 / dt, method="lanczos", starttime=starttime,
